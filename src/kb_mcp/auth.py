...
class KnowledgeBaseKeyVerifier(TokenVerifier):
    ...
    def __init__(self, api_url: str | None = None) -> None:
        ...
        self._cache: dict[str, _CachedUser] = {}
    ...
    async def verify_token(self, token: str) -> AccessToken | None:
        ...
        try:
            ...
            if exc.status not in (401, 403, 404):
                logger.warning("Could not verify key: %s", exc)
            self._cache.pop(token, None)
            return None
        except Exception as exc:  # noqa: BLE001 - network trouble
            logger.warning(
                "Could not reach the Knowledge Base to verify a key: %s", exc
            )
            return None
        ...
        self._cache[token] = _CachedUser(token=access, checked_at=now)
        return access
